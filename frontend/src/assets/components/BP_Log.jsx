  ...
  return (
    <div className="p-2 mb-2 rounded-lg">
      {responseData.bp_log.date.map((date, index) => {
        ...
        // Skip entry if both values are empty
        if (!currentHigh && !currentLow) {
          return null;
        }

        // Show date only if first entry or date changes
        ...
        return (
          <div key={index} className="flex flex-col mb-3">
            ...
            {/* Date Header */}
            {isFirstValueOfDay && (
              <div className="flex items-center mb-2 bg-slate-200 px-2 py-1 rounded-md">
            ...
            <div className="ml-2">
              <div
                className={`text-sm text-gray-700 border border-gray-300 rounded-md px-3 py-2 ${bgColor}`}
              >
